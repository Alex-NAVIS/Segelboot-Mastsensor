...
// Die Daten werden in die globale Struktur "mastdaten" geschrieben.
//
// Hinweis:
...
// Aktualisiert mastdaten mit neuen Positions- und Bewegungsdaten.
// ---------------------------------------------------------
void read_GPS() {
  ...
  if (gps.location.isUpdated()) {
    ...
    if (GPS_AHRS_SYSTEM) {
      ...
      mastdaten.gps_lat = gps.location.lat();   // Breitengrad
      mastdaten.gps_lon = gps.location.lng();   // Längengrad
      mastdaten.gps_speed = gps.speed.knots();  // Geschwindigkeit in kn/h
      mastdaten.gps_kurs = gps.course.deg();    // Kurs über Grund (Grad)
    }
    ...
    if (DEBUG_MODE_GPS) {
      ...
      Serial.print(F("  Lat: "));
      Serial.println(mastdaten.gps_lat, 6);
      Serial.print(F("  Lon: "));
      Serial.println(mastdaten.gps_lon, 6);
      Serial.print(F("  Speed (km/h): "));
      Serial.println(mastdaten.gps_speed, 2);
      Serial.print(F("  Kurs (°): "));
      Serial.println(mastdaten.gps_kurs, 2);
    }
  }
  ...
  if (gps.date.isValid() && gps.time.isValid()) {
    ...
    if (DEBUG_MODE_GPS) {
      ...
      Serial.print(mastdaten.gps_tag);
      Serial.print(F("."));
      Serial.print(mastdaten.gps_monat);
      Serial.print(F("."));
      Serial.println(mastdaten.gps_jahr);


      Serial.print(F("  Zeit (lokal): "));
      Serial.print(mastdaten.gps_stunde);
      Serial.print(F(":"));
      Serial.print(mastdaten.gps_minute);
      Serial.print(F(":"));
      Serial.println(mastdaten.gps_sekunde);
    }
  }
}
...
// - Speichert berechnete lokale Zeit in mastdaten
// ==========================================================
void updateLocalTime(int utcYear, int utcMonth, int utcDay,
  ...
  int tzOffset = int(round(mastdaten.gps_lon / 15.0));
  ...
  // Berechnete lokale Zeit in mastdaten eintragen
  mastdaten.gps_jahr = utcYear;
  mastdaten.gps_monat = utcMonth;
  mastdaten.gps_tag = utcDay;
  mastdaten.gps_stunde = hour;
  mastdaten.gps_minute = minute;
  mastdaten.gps_sekunde = second;
}
...
//   - Schreibt in mastdaten.gps_lat, gps_lon, gps_speed, gps_kurs
...
// Ergebnis → direkt in mastdaten.gps_lat/lon/speed/kurs
// ------------------------------------------------------------------
...
void gps_ahrs() {
  ...
  if (speed_knots < GPS_MIN_VALID_SPEED) {
    ...
    // kurs_deg = mastdaten.gps_kurs; // falls gewünscht
  }

  // Schreib in mastdaten (überschreibt Rohwerte mit gefilterten)
  mastdaten.gps_lat = predLat;
  mastdaten.gps_lon = predLon;
  mastdaten.gps_speed = speed_knots;
  mastdaten.gps_kurs = kurs_deg;
}
